package com.xiaonian.core.common.exception;
...
public class DefaultRestErrorResolver implements RestErrorResolver,
	...
	private RestError buildError(Exception ex, HttpServletRequest request) {
		...
		error.setHttpStatus(this.getHttpStatusByEx(ex));// 设置http状态

		if (ex instanceof RestServiceException) {
			int errorCode = ((RestServiceException) ex).getErrorCode();
			error.setErrorCode(errorCode);
			error.setStatus(((RestServiceException) ex).getStatus());
			error.setMessage(ex.getMessage());
		} else {
			error.setErrorCode(ErrCodeConstants.ERR_3000_SERVER_ERR);
			error.setStatus(Constants.API_STATUS_FAILURE);
//			error.setMessage("未知错误");
	...
	private HttpStatus getHttpStatusByEx(Exception ex) {
		if (this.exceptionMappingDefinitions.containsKey(ex.getClass()
				.getName())) {
			return HttpStatus.valueOf(Integer
					.parseInt(this.exceptionMappingDefinitions.get(ex
							.getClass().getName())));
		} else
	...
	private final Map<String, String> createDefaultExceptionMappingDefinitions() {
		...
		// 409
		applyDef(m, "org.springframework.mapper.DataIntegrityViolationException",
				HttpStatus.CONFLICT);
